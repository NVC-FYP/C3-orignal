...
from model import model_coding
from model.kan import SplineActivation, KANConv2D, KANConv3D
...
class KANSynthesis(hk.Module, model_coding.QuantizableMixin):
  """Synthesis network using KAN: Kolmogorov-Arnold Networks."""
  ...
  def __init__(
      ...
      *,
      ...
      num_residual_layers: int = 0,
      residual_kernel_shape: int = 3,
      num_knots: int = 10,  # KAN-specific: number of knots in spline
      spline_range: float = 3.0,  # KAN-specific: range of spline
      add_layer_norm: bool = False,
      clip_range: tuple[float, float] = (0.0, 1.0),
      is_video: bool = False,
      per_frame_conv: bool = False,
    ...
    Args:
      ...
      residual_kernel_shape: Kernel shape of extra residual conv layers.
        If None, will default to out_channels. Only used when
        num_residual_layers > 0.
      num_knots: Number of knots in the KAN spline activation.
      spline_range: Range of the KAN spline activation.
      add_layer_norm: Whether to add layer norm to the input.
      clip_range: Range at which outputs will be clipped. Defaults to [0, 1]
        which is useful for images and videos.
      is_video: If True, synthesizes a video, otherwise synthesizes an image.
    ...
    net_layers = []
    if is_video:
      kan_conv_cstr = KANConv3D
      conv_cstr = hk.Conv3D
    else:
      kan_conv_cstr = KANConv2D
      conv_cstr = hk.Conv2D
    ...
    for layer_idx, layer_size in enumerate(layers):
      # Use KAN convolutional layers instead of standard + activation
      net_layers += [
          kan_conv_cstr(
    ...
    # Final layer without activation (just like in original)
    net_layers += [
        conv_cstr(
            out_channels,
            kernel_shape=kernel_shape,
    ...
    for i in range(num_residual_layers):
      ...
      if is_last_layer:
        # For the last layer, use regular conv with custom bias init
        core_conv = conv_cstr(
            out_channels,
            kernel_shape=residual_kernel_shape,
            padding='VALID',
        ...
        # For other layers, use KAN conv
        core_conv = kan_conv_cstr(
