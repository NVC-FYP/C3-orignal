...
def get_config() -> config_dict.ConfigDict:
  ...
  exp.dataset.root_dir = '/home/shittyprogrammers/Desktop/Datasets/CLIC2020'
  ...
  exp.quant.kumaraswamy_init_value = 2.
  exp.quant.kumaraswamy_end_value = 1.
  ...
  # Synthesis
  exp.model.synthesis.layers = (24, 24)
  ...
  # Entropy model
  exp.model.entropy.layers = (24, 24)
...
# def get_per_image_sweep(num_seeds=1):
#   base = 'config.experiment_kwargs.config.'
#   return product([
#       sweep('config.random_seed', range(num_seeds)),
#       sweep(base + 'dataset.skip_examples', list(range(41))),
...
# def get_per_image_sweep_paper(num_seeds=1):
#   base = 'config.experiment_kwargs.config.'
#   return product([
#       sweep('config.random_seed', range(num_seeds)),
#       sweep(base + 'model.latents.downsampling_exponents', [
#           (0, 1, 2, 3, 4, 5, 6),  # default: highest grid res = image res
#           (1, 2, 3, 4, 5, 6, 7),  # all grids are downsampled once more
#       ]),
#       zipit([
#           sweep(base + 'model.entropy.layers',
#                       [(12, 12), (18, 18), (24, 24)]),
#           sweep(base + 'model.synthesis.layers',
#                       [(12, 12), (18, 18), (24, 24)]),
#       ]),
#       sweep(base + 'dataset.skip_examples', list(range(41))),
#       sweep(base + 'dataset.num_examples', [1]),
